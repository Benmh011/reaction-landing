...
const LANDING_SECTIONS = [
  ...
  { key: 'opportunities', label: 'Opportunities', categories: ['Opportunities'], description: 'Part-time jobs, summer internships, and graduate schemes — coming soon to the pilot.', gradient: 'linear-gradient(135deg, #064e3b 0%, #059669 100%)', accent: '#34d399', locked: true },
];
          ...
          {LANDING_SECTIONS.map(section => {
            ...
            return (
              <button
                key={section.key}
                disabled={isLocked}
                onClick={isLocked ? undefined : () => { setActiveLandingSection(section.key); setSelectedCategory('all'); setSelectedActivity('all'); setSelectedLocation('all'); setSelectedDate('all'); setSelectedPostedBy('all'); setSelectedSociety('all'); setSelectedCause('all'); setSelectedSector('all'); setSelectedActivityType('all'); setPage('board'); }}
                className={`group text-left rounded-2xl overflow-hidden shadow-md transition-all duration-300 transform relative ${isLocked ? 'cursor-not-allowed' : 'hover:shadow-xl active:scale-[0.98] hover:-translate-y-1'}`}
                style={{ background: section.gradient, opacity: isLocked ? 0.55 : 1, filter: isLocked ? 'grayscale(0.3)' : 'none' }}
                aria-label={isLocked ? `${section.label} — locked, not part of the pilot` : section.label}
              >
                <div className="p-8">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold px-3 py-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.9)' }}>
                      {isLocked ? 'Coming soon' : `${postCount} active post${postCount !== 1 ? 's' : ''}`}
                    </span>
                    {!isLocked && (

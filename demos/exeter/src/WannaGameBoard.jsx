...
const CREST_SRC = "data:image/svg+xml," + encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" fill="none">
  <rect width="100" height="100" rx="12" fill="#003C3C"/>
  <text x="50" y="56" text-anchor="middle" fill="#00A87E" font-size="28" font-weight="700" font-family="system-ui, sans-serif">UoE</text>
  <text x="50" y="78" text-anchor="middle" fill="rgba(0,168,126,0.5)" font-size="9" font-weight="600" font-family="system-ui, sans-serif">click to upload</text>
</svg>`);
...
const LANDING_SECTIONS = [
  { key: 'activities', label: 'Campus', categories: ['Sport', 'Study', 'Board Games'], description: 'Find people to play sport, study together, or join a board game — all on campus.', gradient: 'linear-gradient(135deg, #003C3C 0%, #004D40 100%)', accent: '#00A87E' },
...
// ──── Guild SOCIETIES (real Exeter societies) ────
const GUILD_SOCIETIES = [
  'Debating Society',
  'Biosciences Society',
  'Engineering Society',
  'Christian Union (ExCU)',
  'Computing Society (CompSoc)',
  'Drama Society (DramSoc)',
  'Environmental Society (EnviroSoc)',
  'Gaming Society',
  'Geography Society',
  'Law Society (ExLaw)',
...
const YEAR_GROUP_ENGAGEMENT = [
  { year: 'Year 1', users: 186, pctEngaged: 68, color: '#003C3C' },
  { year: 'Year 2', users: 124, pctEngaged: 51, color: '#2a5a8f' },
  { year: 'Year 3', users: 78, pctEngaged: 37, color: '#00A87E' },
  { year: 'Postgrad', users: 63, pctEngaged: 42, color: '#6b4c8a' },
];

const COMMUTER_VS_CAMPUS = [
  { name: 'On-campus', value: 298, color: '#003C3C' },
  { name: 'Commuter', value: 153, color: '#00A87E' },
];
...
const COMMUNITY_TOP_PARTNERS = [
  { name: 'Exeter City Community Trust', sector: 'Sport for All',  volunteers: 28, hours: 124 },
  { name: "St Petrock's",                sector: 'Homelessness',   volunteers: 22, hours:  96 },
  { name: 'Devon Wildlife Trust',        sector: 'Environment',    volunteers: 19, hours:  78 },
  { name: 'Exeter Community Energy',     sector: 'Sustainability', volunteers: 16, hours:  64 },
  { name: 'Mind Devon',                  sector: 'Mental Health',  volunteers: 15, hours:  62 },
  { name: 'Age UK Exeter',               sector: 'Older People',   volunteers: 14, hours:  58 },
];
...
const OPPS_FUNNEL = [
  { stage: 'Posts Viewed', count: 4280, color: '#003C3C' },
  { stage: 'Posts Saved',  count: 1248, color: '#0d6e4e' },
...
const OPPS_BY_TYPE = [
  { type: 'Part-Time Vacancies', posts: 64, applications: 312, color: '#003C3C' },
...
const OPPS_TOP_EMPLOYERS = [
  { name: 'Met Office',           sector: 'Climate Science', posts: 8, applications: 96 },
  { name: 'Lloyds Banking Group', sector: 'Finance',         posts: 5, applications: 84 },
  { name: 'EDF Energy',           sector: 'Energy',          posts: 6, applications: 72 },
  { name: 'Exeter Science Park',  sector: 'Sciences',        posts: 6, applications: 62 },
  { name: 'Devon County Council', sector: 'Public Sector',   posts: 7, applications: 48 },
  { name: 'Pennon Group',         sector: 'Utilities',       posts: 4, applications: 38 },
];

// 18. OPPS ENGAGEMENT BY YEAR — careers-engagement curve
const OPPS_BY_YEAR = [
  { year: 'Year 1',    users:  38, topInterest: 'Part-Time',         color: '#003C3C' },
...
const INSIGHTS_CATEGORIES = {
  campus: {
    ...
    accent: '#00A87E',
    accentLight: 'rgba(0,168,126,0.08)',
    gradient: 'linear-gradient(135deg, #003C3C 0%, #004D40 60%, #003C3C 100%)',
...
const AnalyticsDashboard = ({ onBack }) => {
  ...
  const tagColor = (text) => ({ NSS:'#00A87E', TEF:'#003C3C', 'A&P':'#6b4c8a', GOS:'#059669', CIVIC:'#be123c' }[text] || '#003C3C');

  const statCard = (label, value, sub, accent, tag) => (
    <div className="bg-white rounded-xl border border-gray-100 p-4 relative overflow-hidden" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.04)', borderLeft: `3px solid ${tag ? tagColor(tag.text) : (accent || '#003C3C')}` }}>
      ...
      <div className="text-2xl font-bold" style={{ color: '#003C3C' }}>{value}</div>
  ...
  return (
    <div className="min-h-screen p-4" style={{ background: 'linear-gradient(135deg, #f0f2f5 0%, #e8ecf1 50%, #f5f0e8 100%)' }}>
      <div className="max-w-6xl mx-auto">
        <button onClick={onBack} className="flex items-center gap-2 font-semibold mb-6 transition-colors text-sm" style={{ color: '#003C3C' }}><ArrowLeft className="w-4 h-4" /> Back to Board</button>
          ...
          {Object.entries(INSIGHTS_CATEGORIES).map(([id, c]) => {
            ...
            return (
              <button key={id} onClick={() => switchCategory(id)} className="rounded-xl p-4 text-left transition-all" style={{ background: isActive ? c.gradient : 'white', border: isActive ? '1px solid transparent' : '1px solid rgba(0,0,0,0.06)', boxShadow: isActive ? '0 4px 12px rgba(0,0,0,0.08)' : '0 1px 3px rgba(0,0,0,0.04)', cursor: 'pointer' }}>
                ...
                <div className="text-base font-bold" style={{ color: isActive ? 'white' : '#003C3C' }}>{c.label}</div>
        ...
        {activeCategory === 'campus' && activeTab === 'overview' && (<>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Monthly Engagement Growth</h2>
              <p className="text-xs text-gray-400 mb-4">Active users, sessions created, and new connections formed</p>
              <ResponsiveContainer width="100%" height={260}><AreaChart data={MONTHLY_ENGAGEMENT} margin={{top:5,right:10,left:-10,bottom:5}}><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9"/><XAxis dataKey="month" tick={{fontSize:12}}/><YAxis tick={{fontSize:12}}/><Tooltip contentStyle={{borderRadius:8,border:'1px solid rgba(30,58,95,0.1)',boxShadow:'0 2px 12px rgba(0,0,0,.06)',fontSize:12}}/><Legend/><Area type="monotone" dataKey="activeUsers" name="Active Users" stroke="#003C3C" fill="#003C3C20" strokeWidth={2.5}/><Area type="monotone" dataKey="sessions" name="Sessions" stroke="#00A87E" fill="#00A87E20" strokeWidth={2.5}/><Area type="monotone" dataKey="newConnections" name="New Connections" stroke="#6b4c8a" fill="#6b4c8a20" strokeWidth={2.5}/></AreaChart></ResponsiveContainer>
            </div>
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Engagement by Activity Category</h2>
              <p className="text-xs text-gray-400 mb-4">Sessions and unique participants across Sport, Study & Board Games</p>
              <ResponsiveContainer width="100%" height={260}><BarChart data={CATEGORY_ENGAGEMENT} margin={{top:5,right:10,left:-10,bottom:5}}><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9"/><XAxis dataKey="category" tick={{fontSize:12}}/><YAxis tick={{fontSize:12}}/><Tooltip contentStyle={{borderRadius:8,border:'1px solid rgba(30,58,95,0.1)',boxShadow:'0 2px 12px rgba(0,0,0,.06)',fontSize:12}}/><Legend/><Bar dataKey="sessions" name="Sessions" fill="#003C3C" radius={[6,6,0,0]}/><Bar dataKey="participants" name="Participants" fill="#00A87E" radius={[6,6,0,0]}/></BarChart></ResponsiveContainer>
            </div>
          </div>
        </>)}
        ...
        {activeCategory === 'campus' && activeTab === 'belonging' && (<>
          <div className="rounded-xl p-4 mb-6" style={{ background: 'rgba(0,168,126,0.06)', border: '1px solid rgba(0,168,126,0.15)' }}>
            <h3 className="font-bold text-sm mb-1" style={{ color: '#003C3C' }}>NSS Theme Relevance</h3>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            {statCard('Self-Reported Belonging', `${avgBelonging}/5`, 'Reaction users (in-app survey)', '#00A87E', null)}
            {statCard('Control Group', '3.1/5', 'non-users comparison', '#9ca3af', null)}
            {statCard('Belonging Differential', '+1.1', 'app users score higher', '#00A87E', null)}
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Self-Reported Belonging Trend</h2>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Cross-Group Interactions</h2>
        ...
        {activeCategory === 'campus' && activeTab === 'participation' && (<>
          <div className="rounded-xl p-4 mb-6" style={{ background: 'rgba(30,58,95,0.04)', border: '1px solid rgba(30,58,95,0.1)' }}>
            <h3 className="font-bold text-sm mb-1" style={{ color: '#003C3C' }}>Access, Participation & Internationalisation</h3>
            <p className="text-xs" style={{ color: '#5a6a7a' }}>Exeter's Strategy 2030 targets 20% international students. OfS requires universities to monitor participation gaps across student groups. This data tracks <strong>international student engagement</strong> alongside year group and commuter metrics for A&P Plan and internationalisation reporting.</p>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>International Student Engagement</h2>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Year Group Engagement</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Commuter vs On-Campus</h2>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center', pointerEvents: 'none' }}>
                  <div style={{ fontSize: '22px', fontWeight: 800, color: '#003C3C', lineHeight: 1 }}>451</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                {[
                  ...
                  <div key={i} style={{ display: 'flex', flexDirection: 'column', padding: '8px 10px', borderRadius: '8px', background: i % 2 === 0 ? 'rgba(30,58,95,0.025)' : 'rgba(0,168,126,0.04)' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: '3px' }}>
                        <div style={{ width: '6px', height: '6px', borderRadius: '2px', background: '#003C3C', flexShrink: 0, marginTop: '2px' }} />
                        <span style={{ fontSize: '13px', fontWeight: 700, color: '#003C3C' }}>{row.campus}{row.unit}</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: '3px' }}>
                        <div style={{ width: '6px', height: '6px', borderRadius: '2px', background: '#00A87E', flexShrink: 0, marginTop: '2px' }} />
                        <span style={{ fontSize: '13px', fontWeight: 700, color: '#00A87E' }}>{row.commuter}{row.unit}</span>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Key Insights for A&P Plan</h2>
              ...
              <div className="space-y-2 mt-2">
                {[
                  ...
                  <div key={i} className="flex items-start gap-2 p-2.5 rounded-lg text-xs font-medium" style={{ background: 'rgba(30,58,95,0.03)', borderLeft: '3px solid #003C3C', color: '#4a5568' }}>
                    <span>{item.text}</span>
                  </div>
                ))}
              </div>
        ...
        {activeCategory === 'campus' && activeTab === 'retention' && (<>
          <div className="rounded-xl p-4 mb-6" style={{ background: 'rgba(0,168,126,0.06)', border: '1px solid rgba(0,168,126,0.15)' }}>
            <h3 className="font-bold text-sm mb-1" style={{ color: '#003C3C' }}>Social Engagement Patterns</h3>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {statCard('Active Users', `${activePct}%`, 'with 1+ sessions in past 14 days', '#00A87E', null)}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Active vs Declining Engagement</h2>
              <p className="text-xs text-gray-400 mb-4">Students with declining social engagement on Reaction over time</p>
              <ResponsiveContainer width="100%" height={280}><AreaChart data={ENGAGEMENT_TREND_DATA} margin={{top:5,right:10,left:-10,bottom:5}}><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9"/><XAxis dataKey="month" tick={{fontSize:12}}/><YAxis tick={{fontSize:12}}/><Tooltip contentStyle={{borderRadius:8,border:'1px solid rgba(30,58,95,0.1)',boxShadow:'0 2px 12px rgba(0,0,0,.06)',fontSize:12}}/><Legend/><Area type="monotone" dataKey="active" name="Active" stroke="#00A87E" fill="#00A87E20" strokeWidth={2.5}/><Area type="monotone" dataKey="declining" name="Declining" stroke="#9ca3af" fill="#9ca3af20" strokeWidth={2.5}/></AreaChart></ResponsiveContainer>
            </div>
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Engagement Frequency Breakdown</h2>
              <p className="text-xs text-gray-400 mb-4">Distribution of users by weekly session frequency</p>
              <div className="space-y-4 mt-4">
                {[
                  { label: '3+ sessions/week', pct: 34, users: 101, color: '#003C3C' },
                  { label: '1–2 sessions/week', pct: 38, users: 113, color: '#2a5a8f' },
                  { label: '<1 session/week', pct: 21, users: 63, color: '#00A87E' },
        ...
        {activeCategory === 'community' && activeTab === 'overview' && (<>
          <div className="rounded-xl p-4 mb-6" style={{ background: 'rgba(190,18,60,0.06)', border: '1px solid rgba(190,18,60,0.15)' }}>
            <h3 className="font-bold text-sm mb-1" style={{ color: '#003C3C' }}>Community Engagement</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Monthly Volunteer Engagement</h2>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Activity Type Breakdown</h2>
        ...
        {activeCategory === 'community' && activeTab === 'social-value' && (<>
          <div className="rounded-xl p-4 mb-6" style={{ background: 'rgba(190,18,60,0.06)', border: '1px solid rgba(190,18,60,0.15)' }}>
            <h3 className="font-bold text-sm mb-1" style={{ color: '#003C3C' }}>Estimated Social Value</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Cumulative Volunteer Hours</h2>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Methodology</h2>
        ...
        {activeCategory === 'community' && activeTab === 'causes' && (<>
          <div className="rounded-xl p-4 mb-6" style={{ background: 'rgba(190,18,60,0.06)', border: '1px solid rgba(190,18,60,0.15)' }}>
            <h3 className="font-bold text-sm mb-1" style={{ color: '#003C3C' }}>Cause Area Distribution</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Volunteers by Cause</h2>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Hours Contributed by Cause</h2>
        ...
        {activeCategory === 'community' && activeTab === 'partners' && (<>
          <div className="rounded-xl p-4 mb-6" style={{ background: 'rgba(190,18,60,0.06)', border: '1px solid rgba(190,18,60,0.15)' }}>
            <h3 className="font-bold text-sm mb-1" style={{ color: '#003C3C' }}>Partner Organisations</h3>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5 mb-6">
            <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Top Partner Organisations</h2>
        ...
        {activeCategory === 'opportunities' && activeTab === 'overview' && (<>
          <div className="rounded-xl p-4 mb-6" style={{ background: 'rgba(5,150,105,0.06)', border: '1px solid rgba(5,150,105,0.15)' }}>
            <h3 className="font-bold text-sm mb-1" style={{ color: '#003C3C' }}>Opportunity & Career Engagement</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Monthly Engagement</h2>
              <p className="text-xs text-gray-400 mb-4">Active users, employer posts, and applications submitted</p>
              <ResponsiveContainer width="100%" height={260}><AreaChart data={OPPS_MONTHLY} margin={{top:5,right:10,left:-10,bottom:5}}><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9"/><XAxis dataKey="month" tick={{fontSize:12}}/><YAxis tick={{fontSize:12}}/><Tooltip contentStyle={{borderRadius:8,border:'1px solid rgba(30,58,95,0.1)',boxShadow:'0 2px 12px rgba(0,0,0,.06)',fontSize:12}}/><Legend/><Area type="monotone" dataKey="activeUsers" name="Active Users" stroke="#003C3C" fill="#003C3C20" strokeWidth={2.5}/><Area type="monotone" dataKey="applications" name="Applications" stroke="#059669" fill="#05966920" strokeWidth={2.5}/><Area type="monotone" dataKey="posts" name="Posts" stroke="#34d399" fill="#34d39920" strokeWidth={2.5}/></AreaChart></ResponsiveContainer>
            </div>
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Opportunity Types</h2>
              <p className="text-xs text-gray-400 mb-4">Posts and applications across part-time, internships, and graduate schemes</p>
              <ResponsiveContainer width="100%" height={260}><BarChart data={OPPS_BY_TYPE} margin={{top:5,right:10,left:-10,bottom:5}}><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9"/><XAxis dataKey="type" tick={{fontSize:10}}/><YAxis tick={{fontSize:12}}/><Tooltip contentStyle={{borderRadius:8,border:'1px solid rgba(30,58,95,0.1)',boxShadow:'0 2px 12px rgba(0,0,0,.06)',fontSize:12}}/><Legend/><Bar dataKey="posts" name="Posts" fill="#003C3C" radius={[6,6,0,0]}/><Bar dataKey="applications" name="Applications" fill="#059669" radius={[6,6,0,0]}/></BarChart></ResponsiveContainer>
            </div>
          </div>
        </>)}
        ...
        {activeCategory === 'opportunities' && activeTab === 'funnel' && (<>
          <div className="rounded-xl p-4 mb-6" style={{ background: 'rgba(5,150,105,0.06)', border: '1px solid rgba(5,150,105,0.15)' }}>
            <h3 className="font-bold text-sm mb-1" style={{ color: '#003C3C' }}>Application Funnel</h3>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5 mb-6">
            <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Funnel Visualisation</h2>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Funnel Insights</h2>
        ...
        {activeCategory === 'opportunities' && activeTab === 'sectors' && (<>
          <div className="rounded-xl p-4 mb-6" style={{ background: 'rgba(5,150,105,0.06)', border: '1px solid rgba(5,150,105,0.15)' }}>
            <h3 className="font-bold text-sm mb-1" style={{ color: '#003C3C' }}>Sector & Type Distribution</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Applications by Sector</h2>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Opportunity Types — Detailed View</h2>
                  ...
                  <div key={t.type} className="rounded-lg p-3" style={{ background: 'rgba(5,150,105,0.04)', border: '1px solid rgba(5,150,105,0.1)' }}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <div className="text-lg font-bold" style={{color:'#003C3C'}}>{t.posts}</div>
                      </div>
                      <div>
        ...
        {activeCategory === 'opportunities' && activeTab === 'employers' && (<>
          <div className="rounded-xl p-4 mb-6" style={{ background: 'rgba(5,150,105,0.06)', border: '1px solid rgba(5,150,105,0.15)' }}>
            <h3 className="font-bold text-sm mb-1" style={{ color: '#003C3C' }}>Employer & Year-Group Engagement</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold mb-1" style={{color:"#003C3C"}}>Top Employers</h2>
...
const ProfilePage = ({ userName, onBack, posts }) => {
  ...
  return (
    <div className="min-h-screen p-4" style={{ background: 'linear-gradient(135deg, #f0f2f5 0%, #e8ecf1 50%, #f5f0e8 100%)' }}><div className="max-w-3xl mx-auto">
      <button onClick={onBack} className="flex items-center gap-2 font-semibold mb-6 transition-colors text-sm" style={{ color: '#003C3C' }}><ArrowLeft className="w-4 h-4" /> Back to Board</button>
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden"><div className="h-28" style={{ background: 'linear-gradient(135deg, #003C3C 0%, #004D40 60%, #00A87E 100%)' }} /><div className="px-8 pb-8">
        <div className="flex flex-col sm:flex-row items-center sm:items-end gap-5 -mt-16 relative z-10"><div className="rounded-full border-4 border-white shadow-lg overflow-hidden" style={{width:128,height:128}}><UserAvatar userName={userName} size={128}/></div><div className="text-center sm:text-left pb-2"><h1 className="text-3xl font-bold text-gray-900">{profile.fullName}</h1><p className="font-medium text-base mt-1" style={{ color: '#00A87E' }}>University of Exeter</p></div></div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-8">
          {[
            { label: 'Full Name', value: profile.fullName, accent: '#003C3C' },
            { label: 'Course', value: profile.course, accent: '#00A87E' },
            { label: 'Age', value: profile.age, accent: '#003C3C' },
            { label: 'Sessions', value: profile.gamesPlayed, accent: '#00A87E' },
          ].map(s => (
        ...
        {userPosts.length > 0 && (<div className="mt-6"><h2 className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#003C3C' }}>Interests</h2><div className="flex flex-wrap gap-2">{[...new Set(userPosts.map(p => p.activity || p.sport))].map(interest=>(<div key={interest} className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 bg-white" style={{ borderLeft: '3px solid rgba(0,168,126,0.4)', boxShadow: '0 1px 3px rgba(0,0,0,0.03)' }}><span className="text-sm font-semibold text-gray-800">{interest}</span></div>))}</div></div>)}
      </div></div>
    </div></div>
  );
};
...
const LoginModal = ({ onClose, onLogin, onSwitchToRegister }) => {
  ...
  return (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-2xl shadow-xl max-w-md w-full overflow-hidden"><div style={{ height: '4px', background: 'linear-gradient(90deg, #003C3C, #00A87E)' }} /><div className="p-6">
    <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold" style={{ color: '#003C3C' }}>Login</h2><button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-2xl leading-none">&times;</button></div>
    <div className="space-y-4"><div><label className="block text-sm font-semibold text-gray-600 mb-1.5">University Email</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="jamie@exeter.ac.uk" className={inputCls}/></div><div><label className="block text-sm font-semibold text-gray-600 mb-1.5">Password</label><input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Enter your password" className={inputCls}/></div></div>
    <button onClick={handleSubmit} className={btnPrimary} style={{ background: '#003C3C', boxShadow: '0 2px 8px rgba(30,58,95,0.25)' }}>Login</button>
    <div className="mt-4 text-center"><p className="text-gray-500 text-sm">Don't have an account?{' '}<button onClick={onSwitchToRegister} className="font-semibold" style={{ color: '#00A87E' }}>Register here</button></p></div>
  </div></div></div>);
};
...
const RegisterModal = ({ onClose, onRegister, onSwitchToLogin }) => {
  ...
  return (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-2xl shadow-xl max-w-md w-full max-h-screen overflow-y-auto"><div style={{ height: '4px', background: 'linear-gradient(90deg, #003C3C, #00A87E)' }} /><div className="p-6">
    <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold" style={{ color: '#003C3C' }}>Create Account</h2><button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-2xl leading-none">&times;</button></div>
    ...
    <div><label className="block text-sm font-semibold text-gray-600 mb-1.5">University Email *</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="jamie@exeter.ac.uk" className={inputCls}/></div>
    <div><label className="block text-sm font-semibold text-gray-600 mb-1.5">University *</label><input type="text" value={uni} onChange={e=>setUni(e.target.value)} placeholder="University of Exeter" className={inputCls}/></div>
    ...
    <button onClick={handleSubmit} className={btnPrimary} style={{ background: '#003C3C', boxShadow: '0 2px 8px rgba(30,58,95,0.25)' }}>Create Account</button>
    <div className="mt-4 text-center"><p className="text-gray-500 text-sm">Already have an account?{' '}<button onClick={onSwitchToLogin} className="font-semibold" style={{ color: '#00A87E' }}>Login here</button></p></div>
  </div></div></div>);
};
...
const CreatePostModal = ({ onClose, onCreate }) => {
  ...
  const [location, setLocation] = useState('Sports Park Hall');
  const [date, setDate] = useState('');
  const [time, setTime] = useState('');
  const [maxPeople, setMaxPeople] = useState(10);
  const [description, setDescription] = useState('');
  const [postedBy, setPostedBy] = useState('Student');
  const [society, setSociety] = useState(GUILD_SOCIETIES[0]);
  ...
  const locationsByCategory = {
    'Sport': ['Sports Park Hall','Russell Seal Fitness Centre','Sports Park 3G Pitch','Exeter Tennis Centre','St Luke\'s Sports Centre','Sports Park MUGA'],
    'Study': ['Forum Library','Amory Building','Harrison Building','Peter Chalk Centre','Café on the Green','Reed Hall'],
    'Board Games': ['Devonshire House','Reed Hall','Peter Chalk Centre','Café on the Green','Ram Bar'],
    'Opportunities': ['Forum','Career Zone','Innovation Centre','Streatham Court','Online'],
    'Community': ['Devonshire House','Forum','Reed Hall','Great Hall','Online'],
  };
  ...
  return (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-2xl shadow-xl max-w-md w-full max-h-screen overflow-y-auto"><div style={{ height: '4px', background: 'linear-gradient(90deg, #003C3C, #00A87E)' }} /><div className="p-6">
    <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold" style={{ color: '#003C3C' }}>Create Post</h2><button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-2xl leading-none">&times;</button></div>
    <div className="space-y-4">
      ...
      <div><label className="block text-sm font-semibold text-gray-600 mb-1.5">Posted As *</label>
        <div className="grid grid-cols-3 gap-2">{['Student','Societies','Staff'].map(g => (
          <button key={g} onClick={() => setPostedBy(g)} className={`py-2 rounded-lg font-semibold text-sm transition-all text-center ${postedBy === g ? 'text-white' : 'bg-gray-50 text-gray-500 border border-gray-200 hover:bg-gray-100'}`} style={postedBy === g ? { background: '#003C3C' } : {}}>
            {g}
          </button>
        ))}</div>
        {postedBy === 'Societies' && (
          <div className="mt-2"><select value={society} onChange={e => setSociety(e.target.value)} className={inputCls}>{GUILD_SOCIETIES.map(s => (<option key={s} value={s}>{s}</option>))}</select></div>
        )}
      </div>
    ...
    <button onClick={handleSubmit} className={btnPrimary} style={{ background: '#00A87E', boxShadow: '0 2px 8px rgba(0,168,126,0.25)' }}>Post to Reaction</button>
  </div></div></div>);
};
  ...
  return (
    <div className="pt-3 border-t border-gray-50">
      ...
      {currentUser && !hasJoined && !isFull && (
        <button onClick={() => onJoin(post.id)} className="w-full py-2 text-white rounded-lg font-semibold text-xs transition-all active:scale-[0.97]" style={{ background: '#003C3C' }}>
          I'll Join
        </button>
      )}
      {currentUser && !hasJoined && isFull && (
...
const BulletinBoardApp = () => {
  ...
  const [posts, setPosts] = useState([
    ...
    { id:1,  user:'Alex Johnson',    category:'Sport', activity:'Basketball',  location:'Sports Park Hall',          date:'2026-02-07', time:'15:00', mode:'team', maxPeople:10, description:'Friendly 5v5, all levels welcome. Bibs provided!', postedBy:'Student' },
    { id:2,  user:'Sarah Martinez',  category:'Sport', activity:'Tennis',      location:'Exeter Tennis Centre',       date:'2026-02-08', time:'14:00', mode:'1v1',  maxPeople:2,  description:'Looking for a rally partner — intermediate level.', postedBy:'Student' },
    { id:3,  user:'Zara Khan',       category:'Sport', activity:'Football',    location:'Sports Park 3G Pitch',      date:'2026-02-09', time:'14:00', mode:'team', maxPeople:10, description:'5-a-side kickabout on the outdoor pitch. Bring water!', postedBy:'Staff' },
    { id:4,  user:'Ryan Cooper',     category:'Sport', activity:'Volleyball',  location:'Russell Seal Fitness Centre',date:'2026-02-10', time:'11:00', mode:'team', maxPeople:12, description:'Beach rules on the indoor court. Staff vs students!', postedBy:'Staff' },
    { id:5,  user:'Emily Davis',     category:'Sport', activity:'Rugby',       location:'Sports Park 3G Pitch',      date:'2026-02-11', time:'16:00', mode:'team', maxPeople:14, description:'Touch rugby, mixed abilities welcome. Far pitch.', postedBy:'Societies', society:'Drama Society (DramSoc)' },
    { id:6,  user:'Tom Wilson',      category:'Sport', activity:'Cricket',     location:'St Luke\'s Sports Centre',     date:'2026-02-12', time:'13:00', mode:'team', maxPeople:22, description:'Casual cricket — pads and bats provided, just turn up!', postedBy:'Societies', society:'Geography Society' },
    { id:7,  user:'Sarah Martinez',  category:'Sport', activity:'Badminton',   location:'Sports Park Hall',          date:'2026-02-09', time:'10:00', mode:'1v1',  maxPeople:2,  description:'Morning singles — just need a partner at a similar level.', postedBy:'Student' },
    { id:8,  user:'Lisa Park',       category:'Sport', activity:'Football',    location:'Sports Park 3G Pitch',      date:'2026-02-13', time:'17:00', mode:'team', maxPeople:10, description:'Women\'s 5-a-side, all abilities. Boots or trainers fine.', postedBy:'Societies', society:'Education Society' },
    { id:9,  user:'Mike Chen',       category:'Sport', activity:'Basketball',  location:'Russell Seal Fitness Centre',date:'2026-02-08', time:'12:00', mode:'team', maxPeople:10, description:'Staff lunchtime hoops — open to postgrads too.', postedBy:'Staff' },

    // ── STUDY (6 activities × mixed postedBy) ──
    { id:10, user:'Mike Chen',       category:'Study', activity:'Group Revision',   location:'Forum Library', date:'2026-02-07', time:'17:00', mode:'group', maxPeople:6,  description:'Covering modules 3–5 for the Thursday exam. Bring notes!', postedBy:'Student' },
    { id:11, user:'Jordan Lee',      category:'Study', activity:'Exam Prep',        location:'Amory Building',            date:'2026-02-08', time:'18:00', mode:'group', maxPeople:5,  description:'Past paper practice session for BIO201.', postedBy:'Student' },
    ...
    { id:15, user:'Zara Khan',       category:'Study', activity:'Dissertation Help',location:'Forum Library', date:'2026-02-12', time:'15:00', mode:'group', maxPeople:3,  description:'Peer feedback on chapter drafts. Bring a printed copy!', postedBy:'Staff' },
    { id:16, user:'Tom Wilson',      category:'Study', activity:'Group Revision',   location:'Reed Hall',    date:'2026-02-10', time:'10:00', mode:'group', maxPeople:8,  description:'Open revision drop-in for ARCH modules. All years welcome.', postedBy:'Societies', society:'Engineering Society' },
    ...
    { id:18, user:'Alex Johnson',    category:'Study', activity:'Project Group',    location:'Amory Building',            date:'2026-02-12', time:'11:00', mode:'group', maxPeople:5,  description:'Computing Society hackathon prep — need backend devs.', postedBy:'Societies', society:'Computing Society (CompSoc)' },
    ...
    { id:19, user:'Emily Davis',     category:'Board Games', activity:'Catan',              location:'Devonshire House',   date:'2026-02-09', time:'16:00', mode:'group', maxPeople:4,  description:'Settlers + Seafarers expansion. Beginners very welcome.', postedBy:'Societies', society:'Tabletop Gaming Society' },
    { id:20, user:'Tom Wilson',      category:'Board Games', activity:'Chess',              location:'Reed Hall',   date:'2026-02-08', time:'12:00', mode:'1v1',   maxPeople:2,  description:'Casual lunchtime game. Any skill level.', postedBy:'Student' },
    ...
    { id:22, user:'Lisa Park',       category:'Board Games', activity:'Risk',               location:'Devonshire House',   date:'2026-02-11', time:'17:00', mode:'group', maxPeople:5,  description:'World domination session. Expect it to last 3+ hours.', postedBy:'Societies', society:'Gaming Society' },
    ...
    { id:24, user:'Zara Khan',       category:'Board Games', activity:'Poker',              location:'Reed Hall',   date:'2026-02-12', time:'20:00', mode:'group', maxPeople:6,  description:'Texas hold\'em, chips only — no real money. Snacks provided.', postedBy:'Staff' },
    ...
    { id:27, user:'Ryan Cooper',     category:'Board Games', activity:'Catan',              location:'Devonshire House',   date:'2026-02-14', time:'14:00', mode:'group', maxPeople:4,  description:'Staff board game social — all welcome, not just staff!', postedBy:'Staff' },

    // ── OPPORTUNITIES (with listing details for dedicated pages) ──
    { id:30, user:'Zara Khan',       category:'Opportunities', activity:'Part Time Vacancies',  location:'Career Zone',         date:'2026-02-07', time:'09:00', mode:'group', maxPeople:20, description:'Student Ambassador roles now open — flexible hours, great for your CV!', postedBy:'Staff', sector:'University', activityType:'Customer-Facing',
      listingDetails: { employer: 'University of Exeter', hours: '6–12 hrs/week', pay: '£12.00/hr', duration: 'Ongoing (term-time)', deadline: '2026-02-28', requirements: 'Current UoE student, confident communicator, enthusiastic about the university.', responsibilities: 'Leading campus tours for prospective students and families, representing the university at open days and offer-holder events, sharing your student experience authentically.', howToApply: 'Submit your CV and a short cover letter via the Career Zone portal.' }},
    { id:31, user:'Emily Davis',     category:'Opportunities', activity:'Part Time Vacancies',  location:'Forum',               date:'2026-02-09', time:'10:00', mode:'group', maxPeople:15, description:'Library assistant positions available. 10–15 hrs/week during term.', postedBy:'Staff', sector:'University', activityType:'Admin & Operations',
      listingDetails: { employer: 'University of Exeter Library Services', hours: '10–15 hrs/week', pay: '£11.44/hr', duration: 'Term-time only', deadline: '2026-02-20', requirements: 'Attention to detail, ability to work independently, familiarity with library systems is a plus.', responsibilities: 'Shelving and organising materials, assisting students with enquiries, processing inter-library loans, and maintaining study spaces.', howToApply: 'Apply online via the university jobs portal. Reference: LIB-2026-PT.' }},
    { id:32, user:'Mike Chen',       category:'Opportunities', activity:'Summer Internships',   location:'Innovation Centre',   date:'2026-02-08', time:'14:00', mode:'group', maxPeople:30, description:'Tech internships fair — local startups and national firms. Bring your CV!', postedBy:'Staff', sector:'Tech & Engineering', activityType:'Careers Fair',
      listingDetails: { employer: 'Various (Careers Fair)', hours: 'Full-time (summer)', pay: 'Varies by employer', duration: '8–12 weeks', deadline: '2026-02-08 (event date)', requirements: 'Open to all year groups. Bring printed CVs and be ready to network.', responsibilities: 'Attend employer stands, participate in speed networking sessions, submit applications on the day or shortly after.', howToApply: 'Register attendance via the Career Zone events page. No pre-application needed.' }},
    { id:33, user:'Alex Johnson',    category:'Opportunities', activity:'Summer Internships',   location:'Online',              date:'2026-02-10', time:'11:00', mode:'group', maxPeople:50, description:'Deadline reminder: Civil Service Summer Diversity Internship closes Feb 14.', postedBy:'Societies', society:'Law Society (ExLaw)', sector:'Public Sector', activityType:'Diversity Scheme',
    ...
    { id:34, user:'Tom Wilson',      category:'Opportunities', activity:'Graduate Schemes',     location:'Streatham Court',     date:'2026-02-11', time:'13:00', mode:'group', maxPeople:25, description:'Employer panel: Deloitte, Teach First & NHS Graduate Management. Q&A after.', postedBy:'Staff', sector:'Consulting', activityType:'Employer Panel',
    ...
    { id:35, user:'Jordan Lee',      category:'Opportunities', activity:'Graduate Schemes',     location:'Career Zone',         date:'2026-02-12', time:'15:00', mode:'group', maxPeople:20, description:'1-to-1 CV and application review drop-in. No booking needed.', postedBy:'Staff', sector:'University', activityType:'Application Support',
      listingDetails: { employer: 'University of Exeter Careers Service', hours: 'Drop-in (30 min slots)', pay: 'Free service', duration: 'One-off session', deadline: 'No deadline — drop in', requirements: 'Bring a draft CV or application you are working on. Open to all students and recent graduates.', responsibilities: 'A careers adviser will review your CV, cover letter, or application form and provide tailored feedback to strengthen it.', howToApply: 'Just turn up at the Career Zone during drop-in hours. First come, first served.' }},
    { id:38, user:'Ryan Cooper',     category:'Opportunities', activity:'Part Time Vacancies',  location:'Online',              date:'2026-02-14', time:'09:00', mode:'group', maxPeople:10, description:'Paid research assistant needed for Psychology dept study. 8 hrs/week.', postedBy:'Staff', sector:'Research', activityType:'Research Role',
      listingDetails: { employer: 'Department of Psychology, University of Exeter', hours: '8 hrs/week', pay: '£12.50/hr', duration: '6 weeks (March–April)', deadline: '2026-02-21', requirements: 'Psychology or related degree student (Year 2+). Experience with Qualtrics or SPSS preferred but not essential.', responsibilities: 'Recruiting participants, administering questionnaires, entering and cleaning data, and supporting the lead researcher with literature searches.', howToApply: 'Email r.cooper@exeter.ac.uk with your CV and a brief expression of interest.' }},

    // ── COMMUNITY (4 activity types × mixed postedBy) ──
    { id:36, user:'Sarah Martinez',  category:'Community', activity:'Volunteering',     location:'Forum',               date:'2026-02-09', time:'10:00', mode:'group', maxPeople:30, description:'Beach clean volunteering trip to Exmouth — transport provided!', postedBy:'Societies', society:'Environmental Society (EnviroSoc)', cause:'Environment', activityType:'Outdoor Conservation' },
    { id:37, user:'Lisa Park',       category:'Community', activity:'Volunteering',     location:'Devonshire House',    date:'2026-02-13', time:'16:00', mode:'group', maxPeople:15, description:'Code Club mentor training — help local schoolchildren learn to code.', postedBy:'Societies', society:'Computing Society (CompSoc)', cause:'Education & Youth', activityType:'Tutoring & Mentoring' },
    { id:39, user:'Emily Davis',     category:'Community', activity:'Social Events',    location:'Reed Hall',           date:'2026-02-08', time:'19:00', mode:'group', maxPeople:40, description:'International food night — bring a dish from home! Sign up sheet at the Guild.', postedBy:'Societies', society:'Geography Society', cause:'Cultural & Inclusion', activityType:'Cultural Nights' },
    { id:40, user:'Alex Johnson',    category:'Community', activity:'Social Events',    location:'Great Hall',          date:'2026-02-14', time:'20:00', mode:'group', maxPeople:50, description:'Valentine\'s quiz night — teams of 4-6, prizes for winners!', postedBy:'Staff', cause:'Social & Wellbeing', activityType:'Quiz & Games' },
    { id:41, user:'Zara Khan',       category:'Community', activity:'Fundraising',      location:'Forum',               date:'2026-02-10', time:'11:00', mode:'group', maxPeople:20, description:'RAG week bake sale — all donations go to Devon Air Ambulance.', postedBy:'Societies', society:'Medical Sciences Society', cause:'Health', activityType:'Bake Sales' },
    { id:42, user:'Tom Wilson',      category:'Community', activity:'Campaigns',        location:'Devonshire House',    date:'2026-02-11', time:'14:00', mode:'group', maxPeople:25, description:'Sustainability swap shop — bring old clothes, take something new to you!', postedBy:'Societies', society:'Environmental Society (EnviroSoc)', cause:'Environment', activityType:'Sustainability' },
  ...
  const handleLogin = (email) => { setUserProfile({firstName:'Rhys',lastName:'',email,university:'University of Exeter'}); setIsLoggedIn(true); setShowLoginModal(false); };
    ...
    return (
      <div className="min-h-screen p-4" style={{ background: 'linear-gradient(135deg, #f0f2f5 0%, #e8ecf1 50%, #f5f0e8 100%)' }}>
        <div className="max-w-2xl mx-auto">
          <button onClick={() => setApplyingTo(null)} className="flex items-center gap-1.5 text-sm font-semibold mb-6 transition-all hover:opacity-70" style={{ color: '#003C3C' }}>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h2 className="text-lg font-bold mb-1" style={{ color: '#003C3C' }}>Submit your application</h2>
    ...
    return (
      <div className="min-h-screen p-4" style={{ background: 'linear-gradient(135deg, #f0f2f5 0%, #e8ecf1 50%, #f5f0e8 100%)' }}>
        <div className="max-w-3xl mx-auto">
          <button onClick={() => setViewingListing(null)} className="flex items-center gap-1.5 text-sm font-semibold mb-6 transition-all hover:opacity-70" style={{ color: '#003C3C' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div className="md:col-span-2 space-y-5">
              {ld.responsibilities && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                  <h3 className="text-sm font-bold uppercase tracking-wider mb-3" style={{ color: '#003C3C' }}>What you will do</h3>
                  <p className="text-sm text-gray-600 leading-relaxed">{ld.responsibilities}</p>
                </div>
              )}
              {ld.requirements && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                  <h3 className="text-sm font-bold uppercase tracking-wider mb-3" style={{ color: '#003C3C' }}>Requirements</h3>
                  <p className="text-sm text-gray-600 leading-relaxed">{ld.requirements}</p>
                </div>
              )}
              {ld.howToApply && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                  <h3 className="text-sm font-bold uppercase tracking-wider mb-3" style={{ color: '#003C3C' }}>How to apply</h3>
            ...
            <div className="space-y-4">
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                <h3 className="text-xs font-bold uppercase tracking-wider mb-4" style={{ color: '#00A87E' }}>Details</h3>
  ...
  if (page === 'home') return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #f0f2f5 0%, #e8ecf1 50%, #f5f0e8 100%)' }}>
      <div className="max-w-5xl mx-auto p-4">
        {/* Header */}
        <div className="rounded-2xl shadow-lg mb-8 overflow-hidden" style={{ background: 'linear-gradient(135deg, #003C3C 0%, #004D40 60%, #003C3C 100%)' }}>
          <div className="p-6">
            <div className="flex justify-between items-center flex-wrap gap-4">
              <div className="flex items-center gap-4">
                <label className="w-14 h-14 flex-shrink-0 cursor-pointer">
                  <img src={logoSrc} alt="Logo" className="w-14 h-14 object-contain rounded-lg" style={{ border: '2px solid rgba(0,168,126,0.6)' }} />
                ...
                <div>
                  ...
                  <p className="text-xs font-medium" style={{ color: '#00A87E' }}>University of Exeter Connect</p>
                </div>
              </div>
              <div className="flex gap-2.5 flex-wrap items-center">
                ...
                {isLoggedIn ? (<>
                  <span className="px-3.5 py-2 rounded-lg text-sm font-semibold" style={{ background: 'rgba(0,168,126,0.15)', color: '#00A87E', border: '1px solid rgba(0,168,126,0.25)' }}>Welcome, {userProfile.firstName}</span>
          ...
          <div style={{ height: '3px', background: 'linear-gradient(90deg, #00A87E 0%, #00C896 50%, #00A87E 100%)' }} />
        </div>

        {/* Hero tagline */}
        <div className="text-center mb-10">
          <h2 className="text-3xl font-bold mb-2" style={{ color: '#003C3C', letterSpacing: '-0.03em' }}>What do you wanna do?</h2>
          <p className="text-gray-500 text-sm max-w-md mx-auto">Find people, join in, and make the most of your time at Exeter.</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 sm:p-6">
          ...
          <div className="grid grid-cols-3 gap-3 sm:gap-6 text-center">
            <div>
              <div className="text-xl sm:text-2xl font-bold" style={{ color: '#003C3C' }}>{posts.length}</div>
              <div className="text-[10px] sm:text-xs text-gray-400 font-medium mt-1">Total Posts</div>
            </div>
            <div>
              <div className="text-xl sm:text-2xl font-bold" style={{ color: '#00A87E' }}>{new Set(posts.map(p => p.user)).size}</div>
          ...
          <button
            ...
            style={{
              color: showCategoryStats ? '#003C3C' : '#6b7280',
              background: showCategoryStats ? 'rgba(0,168,126,0.06)' : 'rgba(30,58,95,0.025)',
              border: showCategoryStats ? '1px solid rgba(0,168,126,0.2)' : '1px solid rgba(30,58,95,0.08)',
            }}
          >
          ...
          {showCategoryStats && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4">
              {/* Campus */}
              <div className="rounded-lg p-3" style={{ background: 'rgba(0,168,126,0.05)', borderLeft: '3px solid #00A87E' }}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: '#003C3C' }}>Campus</span>
                  <span className="text-[10px] font-semibold" style={{ color: '#00A87E' }}>{posts.filter(p => ['Sport','Study','Board Games'].includes(p.category)).length} posts</span>
                </div>
                <div className="grid grid-cols-3 gap-2 mt-2">
                  <div>
                    <div className="text-base font-bold" style={{ color: '#003C3C' }}>{posts.filter(p => p.category === 'Sport').length}</div>
                    <div className="text-[9px] text-gray-500 uppercase tracking-wider mt-0.5">Sport</div>
                  </div>
                  <div>
                    <div className="text-base font-bold" style={{ color: '#003C3C' }}>{posts.filter(p => p.category === 'Study').length}</div>
                    <div className="text-[9px] text-gray-500 uppercase tracking-wider mt-0.5">Study</div>
                  </div>
                  <div>
                    <div className="text-base font-bold" style={{ color: '#003C3C' }}>{posts.filter(p => p.category === 'Board Games').length}</div>
  ...
  return (
    <div className="min-h-screen p-4" style={{ background: 'linear-gradient(135deg, #f0f2f5 0%, #e8ecf1 50%, #f5f0e8 100%)' }}>
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <div className="rounded-2xl shadow-lg mb-6 overflow-hidden" style={{ background: 'linear-gradient(135deg, #003C3C 0%, #004D40 60%, #003C3C 100%)' }}>
          <div className="p-6">
            <div className="flex justify-between items-center flex-wrap gap-4">
              <div className="flex items-center gap-4">
                <label className="w-14 h-14 flex-shrink-0 cursor-pointer">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-wrap md:flex-nowrap items-stretch md:items-center w-full">
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      borderBottom: isActive ? '2px solid #00A87E' : isOpen ? '2px solid rgba(30,58,95,0.15)' : '2px solid transparent',
                      background: isOpen ? 'rgba(30,58,95,0.02)' : isActive ? 'rgba(30,58,95,0.015)' : 'transparent',
                    }}
                  >
                    <span style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: isActive ? '#003C3C' : '#aab0b8', whiteSpace: 'nowrap' }}>{f.label}</span>
                    {isActive && (
                      <span
                        style={{
                          fontSize: '9px', fontWeight: 700, color: '#003C3C',
                          background: 'rgba(0,168,126,0.12)', padding: '2px 6px', borderRadius: '4px', lineHeight: 1.2,
                          maxWidth: '100%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                        }}
                        title={f.fmt(f.value)}
                      >{f.fmt(f.value)}</span>
                    )}
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={isActive ? '#00A87E' : '#aab0b8'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ transition: 'transform 0.2s', transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)', flexShrink: 0 }}><path d="M6 9l6 6 6-6"/></svg>
                  </div>
                  {isOpen && (
                    <div style={{
                      ...
                      <div style={{ padding: '6px 10px 4px', marginBottom: '2px' }}>
                        <span style={{ fontSize: '9px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#00A87E' }}>{f.label}</span>
                      </div>
                      {f.opts.map(o => {
                        const sel = f.value === o;
                        return (
                          <div
                            ...
                            style={{
                              ...
                              color: sel ? '#003C3C' : '#6b7280',
                              background: sel ? 'rgba(30,58,95,0.05)' : 'transparent',
                              borderLeft: sel ? '2px solid #00A87E' : '2px solid transparent',
                            }}
                            onMouseEnter={(e) => { if (!sel) e.currentTarget.style.background = 'rgba(0,168,126,0.05)'; }}
                            onMouseLeave={(e) => { if (!sel) e.currentTarget.style.background = 'transparent'; }}
                          >
                            <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.fmt(o)}</span>
                            {sel && <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#00A87E" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" style={{flexShrink:0}}><path d="M5 13l4 4L19 7"/></svg>}
                          </div>
                        );
                      })}
                    </div>
            ...
            {(selectedCategory!=='all'||selectedPostedBy!=='all'||selectedSociety!=='all'||selectedActivity!=='all'||selectedLocation!=='all'||selectedDate!=='all'||selectedCause!=='all'||selectedSector!=='all'||selectedActivityType!=='all') && (
              <div className="basis-full md:basis-auto md:flex-shrink-0" style={{ padding: '8px 10px', borderTop: '1px solid rgba(30,58,95,0.08)', display: 'flex', justifyContent: 'center' }}>
                <div onClick={()=>{setSelectedCategory('all');setSelectedPostedBy('all');setSelectedSociety('all');setSelectedActivity('all');setSelectedLocation('all');setSelectedDate('all');setSelectedCause('all');setSelectedSector('all');setSelectedActivityType('all');setOpenFilter(null);}} style={{ padding: '5px 10px', fontSize: '11px', fontWeight: 600, borderRadius: '6px', cursor: 'pointer', color: '#00A87E', background: 'rgba(0,168,126,0.08)', whiteSpace: 'nowrap' }}>✕ Clear filters</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-3">
          {filteredPosts.length === 0 ? (
            ...
            const catColors = {
              'Sport':       { accent: '#003C3C', light: 'rgba(30,58,95,0.06)', text: '#003C3C' },
              'Study':       { accent: '#00A87E', light: 'rgba(0,168,126,0.08)', text: '#007D69' },
              ...
              <div className="flex">
                ...
                <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'stretch', flex: 1 }}>
                  ...
                  <div style={{ flex: 1, padding: '16px', minWidth: 0 }}>
                    ...
                    <div className="flex items-center gap-0 text-xs text-gray-500 mb-3 flex-wrap">
                      ...
                      <div className="flex items-center gap-1.5 cursor-pointer hover:text-gray-800 transition-colors pr-3.5" onClick={() => setViewingProfile(post.user)}>
                        ...
                        <span className="px-1.5 py-0.5 rounded text-xs font-semibold" style={{ background: post.postedBy === 'Staff' ? 'rgba(30,58,95,0.08)' : post.postedBy === 'Societies' ? 'rgba(107,76,138,0.08)' : 'rgba(0,168,126,0.1)', color: post.postedBy === 'Staff' ? '#003C3C' : post.postedBy === 'Societies' ? '#6b4c8a' : '#007D69', fontSize: '10px' }}>{post.postedBy === 'Societies' && post.society ? post.society : (post.postedBy || 'Student')}</span>
                      </div>
                      )}
                      ...
                      <div className="flex items-center gap-1.5 px-3.5 py-1 rounded-md transition-colors" style={{ borderLeft: '1px solid #e5e7eb' }}>
                        <MapPin className="w-3 h-3" style={{ color: '#00A87E' }} />
                        <span>{post.location}</span>
                      </div>
                      <div className="flex items-center gap-1.5 px-3.5 py-1 rounded-md transition-colors" style={{ borderLeft: '1px solid #e5e7eb' }}>
                        <Calendar className="w-3 h-3" style={{ color: '#00A87E' }} />
                  ...
                  <div style={{ width: '30%', minWidth: '120px', maxWidth: '220px', background: 'rgba(30,58,95,0.015)', borderLeft: '1px solid rgba(0,0,0,0.04)', padding: '14px', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
                    <div className={`text-xs font-semibold uppercase tracking-wider mb-1.5 ${post.category === 'Opportunities' && post.listingDetails ? 'cursor-pointer hover:opacity-70 transition-opacity' : ''}`} style={{ color: '#00A87E', opacity: 0.7 }} onClick={() => { if (post.category === 'Opportunities' && post.listingDetails) setViewingListing(post); }}>{post.category === 'Opportunities' && post.listingDetails ? 'About \u203A' : 'About'}</div>
        ...
        {!isLoggedIn && filteredPosts.length > 0 && (
          <div className="mt-6 rounded-xl p-6 text-center" style={{ background: 'linear-gradient(135deg, #003C3C, #004D40)', border: '1px solid rgba(0,168,126,0.2)' }}>
            <p className="text-white font-semibold text-base mb-3" style={{ opacity: 0.95 }}>Want to join? Login or register with your Exeter email to get started.</p>
            <div className="flex gap-3 justify-center flex-wrap">
              <button onClick={() => setShowLoginModal(true)} className="px-6 py-2.5 rounded-lg font-semibold text-sm text-white transition-all active:scale-[0.97]" style={{ background: '#00A87E', boxShadow: '0 2px 8px rgba(0,168,126,0.3)' }}>Login</button>
